...
namespace MedPod
{
    public class WorkGiver_DoctorCarryFromBedToMedPod : WorkGiver_DoctorRescueToMedPod
	{
		...
		public override bool HasJobOnThing(Pawn pawn, Thing t, bool forced = false)
		{
			...
			if (patient == null || patient == pawn || patient.Faction != pawn.Faction || patient.CurrentBed()?.def.thingClass == typeof(Building_BedMedPod) || patient.health.surgeryBills.Bills.Any(x => x.suspended == false) || !pawn.CanReserve(patient, 1, -1, null, forced) || GenAI.EnemyIsNear(patient, MinDistFromEnemy))
			{
				return false;
			}
